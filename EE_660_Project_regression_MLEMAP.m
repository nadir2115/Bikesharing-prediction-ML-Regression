...
%Non-linear regression
    nl=1;     x = x.^nl;
xtest= x(ceil(m*0.9):m,:); 
ytest= y(ceil(m*0.9):m); % Separating test set


%% Y casual data
tic
% MLE
p_error= 100000000000000;
for k=1:1000
rowindices= randperm(floor(m*0.9));
TData = Data(rowindices, :);
xtrain= TData(1:floor(m*0.8),1:n-3); 
ytrain= TData(1:floor(m*0.8),n-2); %Creating train set
xvalidation= TData(ceil(m*0.8):floor(m*0.9),1:n-3);  % Creating Validation sets
yvalidation= TData(ceil(m*0.8):floor(m*0.9),n-2); 
...
for i=1:100
    ...
    yvalidation= Data(ceil(m*0.8):floor(m*0.9),n-2); % Creating Validation set
    xtrain= Data(1:floor(m*0.8),1:n-3);  %Creating train set
    ytrain= Data(1:floor(m*0.8),n-2); 
...
TData = Data(rowindices, :);
xtrain= TData(1:floor(m*0.8),1:n-3); 
ytrain= TData(1:floor(m*0.8),n-1); %Creating train set
xvalidation= TData(ceil(m*0.8):floor(m*0.9),1:n-3);  % Creating Validation sets
yvalidation= TData(ceil(m*0.8):floor(m*0.9),n-1); 
xtxtrain= (transpose(xtrain))*xtrain;
wMLEk = (inv(xtxtrain))*(transpose(xtrain))*ytrain;
ymle_est= xvalidation*wMLEk; 
MSE_mle = immse(ymle_est,yvalidation); %MSE for MLE W
